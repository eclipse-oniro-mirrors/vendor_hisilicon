...
#define THREE_HUNDRED_AND_SIXTY 360
...
{
    uint16_t loc_angle;
    if (par_deg <= THREE_HUNDRED_AND_SIXTY) {
    loc_angle = par_deg;
    } else {
    loc_angle = par_deg % THREE_HUNDRED_AND_SIXTY;
    loc_angle = ((par_deg != 0)?par_deg:THREE_HUNDRED_AND_SIXTY);
    }
    return loc_angle;
}
...
void ssd1306_DrawArc(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep,)
{
    ...
    count = (ssd1306_NormalizeTo0_360(start_angle) * CIRCLE_APPROXIMATION_SEGMENTS) / THREE_HUNDRED_AND_SIXTY;
    approx_segments = (loc_sweep * CIRCLE_APPROXIMATION_SEGMENTS) / THREE_HUNDRED_AND_SIXTY;

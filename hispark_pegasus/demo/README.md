# Hi3861V100实验开发指导<a name="ZH-CN_TOPIC_0000001130176841"></a>
前言：在学习实验之前，请学习一下理论知识，[前往理论知识课程](http://developer.huawei.com/consumer/cn/training/course/introduction/C101641968823265204)
## Hi3861V100硬件介绍及搭建<a name="section11660541593"></a>
-    [Hi3861V100硬件搭建](doc/硬件环境搭建.md)
## Hi3861V100应用场景介绍<a name="section11660541593"></a>
-    [Hi3861V100应用场景介绍](doc/Hi3861V100应用场景介绍.md)
## 开发环境搭建<a name="section11660541593"></a>
-    [搭建IDE工具开发环境](https://device.harmonyos.com/cn/docs/documentation/guide/ide-install-windows-ubuntu-0000001194073744)
-    [源码获取](https://device.harmonyos.com/cn/docs/documentation/guide/sourcecode-acquire-0000001050769927)
-    [打开工程/源码](https://device.harmonyos.com/cn/docs/documentation/guide/open_project-0000001071680043)
-    [编译Hi3861V100开发板源码](https://device.harmonyos.com/cn/docs/documentation/guide/ide-hi3861v100-compile-0000001192526021)
-    [Hi3861V100开发板烧录](https://device.harmonyos.com/cn/docs/documentation/guide/ide-hi3861-upload-0000001051668683)
-    [IDE串口工具的使用](doc/IDE串口工具指导.md)
-    [运行第一个程序Hello world](hello_world_demo/hello_world_demo.md)
-    [环境搭建FAQ](http://gitee.com/openharmony/docs/blob/master/zh-cn/device-dev/quick-start/quickstart-lite-steps-hi3861-faqs.md)
## WiFi_IoT常用接口案例<a name="section11660541593"></a>
-    [I2C通信介绍](doc/I2C通信介绍.md)
-    [UART通信介绍](doc/UART通信介绍.md)
-    [PWM接口介绍](doc/PWM接口介绍.md)
## WiFi_IoT基础控制实验<a name="section11660541593"></a>
-    [led实验](led_demo/led_demo.md)
-    [蜂鸣器实验](beep_demo/beep_demo.md)
## WiFi_IoT网络通信实验<a name="section11660541593"></a>
-    [AT指令应用](doc/AT指令应用.md)
-    [LWIP协议的TCP/IP通信](lwip_demo/lwip_demo.md)
-    [MQTT的通信实验](mqtt_demo/mqtt_demo.md)
-    [Coap协议的通信实验](coap_demo/coap_demo.md)
## WiFi_IoT应用实践实验<a name="section11660541593"></a>
-    [Histreaming APP的智慧交通灯](histreaming_demo/histreaming_demo.md)
-    [华为云的智能交通灯上报](oc_demo/oc_demo.md)
-    [腾讯云的微信小程序开发及Hi3861板端互联](iottencent_demo/iottencent_demo.md)
-    [微信小程序基于UDP局域网和Hi3861板端互联](udp_demo/udp_demo.md)
## WiFi_IoT工程实验<a name="section11660541593"></a>
-    [交通灯实验](traffic_light_demo/traffic_light_demo.md)
-    [环境检测实验](environment_demo/environment_demo.md)
-    [NFC实验](nfc_demo/nfc_demo.md)
## 附录<a name="section11660541593"></a>
-    [Hi3861V100 BootLoader的启动与运行](doc/BootLoader的启动与运行.md)
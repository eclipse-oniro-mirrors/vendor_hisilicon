...
{
    sntp_retry_timeout = SNTP_RETRY_TIMEOUT
}
 ...
 */
static void sntp_retry(int *arg)
{
    LWIP_UNUSED_ARG(arg);
 ...
 * @param arg is unused (only necessary to conform to sys_timeout)
 */
static void sntp_try_next_server(int *arg)
{
    u8_t old_server, i;
    LWIP_UNUSED_ARG(arg);
    ...
    sntp_current_server++;
    if (SNTP_MAX_SERVERS >= sntp_current_server) {
 ...
 */
static void sntp_dns_found(const char *hostname, const ip_addr_t *ipaddr, int *arg)
{
 ...
 * @param arg is unused (only necessary to conform to sys_timeout)
 */
static void sntp_request(int *arg)
{

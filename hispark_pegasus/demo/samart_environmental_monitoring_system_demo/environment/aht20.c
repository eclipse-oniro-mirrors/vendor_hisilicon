...
uint32_t AHT20_GetMeasureResult(float* temp, float* humi)
{
    ...
    humiRaw = (humiRaw << EIGHT) | buffer[TWO];
    humiRaw = (humiRaw << FOUR) | ((buffer[THREE] & 0xF0) >> FOUR);
    *humi = humiRaw / (float)AHT20_RESLUTION * ONE_HUNDRED;

    uint32_t tempRaw = buffer[3] & 0x0F;
    tempRaw = (tempRaw << EIGHT) | buffer[FOUR];
    tempRaw = (tempRaw << EIGHT) | buffer[FIVE];
    *temp = tempRaw / (float)AHT20_RESLUTION * TWO_HUNDRED - FIFTY;
    return IOT_SUCCESS;
}

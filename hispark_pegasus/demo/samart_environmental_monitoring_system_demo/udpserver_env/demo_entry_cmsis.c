...
int ConnectToWifi(const char* ssid, const char* key, const int type)
{
    ...
    if (strcpy_s(config.ssid, sizeof(config.ssid), ssid)) {
}
    if (strcpy_s(config.preSharedKey, sizeof(config.preSharedKey), key)) {
}
...
int StartConfig(void)
{
    ...
    if (strcpy_s(config.ssid, sizeof(config.ssid), "HarmonyOS-AP")) {
}

...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size)
{
    ...
    for (size_t i = 0; i < buff_size; i++) {
        data[i*TWO] = SSD1306_CTRL_DATA | SSD1306_MASK_CONT;
        data[i*TWO+1] = buffer[i];
    }
    data[(buff_size - 1) * TWO] = SSD1306_CTRL_DATA;
...
void ssd1306_UpdateScreen(void)
{
    ...
    uint32_t count = 0;
    uint8_t data[sizeof(cmd)*TWO + SSD1306_BUFFER_SIZE + 1] = {};
    ...
    memcpy_s(&data[count], SSD1306_Buffer, sizeof(SSD1306_Buffer));
...
void ssd1306_DrawPixel(uint8_t x, uint8_t y, SSD1306_COLOR color)
{
    ...
    if (SSD1306.Inverted) {
        color_1 = (SSD1306_COLOR)!color_1;
    }

    // Draw in the right color
    if (color_1 == White) {
        SSD1306_Buffer[x + (y / EIGHT) * SSD1306_WIDTH] |= 1 << (y % EIGHT);
    } else {
        SSD1306_Buffer[x + (y / EIGHT) * SSD1306_WIDTH] &= ~(1 << (y % EIGHT));
    }
}
...
char ssd1306_DrawChar(char ch, FontDef Font, SSD1306_COLOR color)
{
    ...
    if (ch < THIRTY_TWO || ch > ONE_HUNDRED_TWENTY_SIX) {
        return 0;
    }
    ...
    for (i = 0; i < Font.FontHeight; i++) {
        b = Font.data[(ch - THIRTY_TWO) * Font.FontHeight + i];
...
void ssd1306_DrawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, SSD1306_COLOR color)
{
    uint8_t y = y1;
    uint8_t x = x1;
    int32_t deltaX = abs(x2 - x);
    int32_t deltaY = abs(y2 - y);
    int32_t signX = ((x < x2) ? 1 : -1);
    int32_t signY = ((y < y2) ? 1 : -1);
...
{
    return par_deg * THREE_POINT_ONE_FOUR / ONE_HUNDRED_EIGHTY;
}
/* Normalize degree to [0;360] */
static uint16_t ssd1306_NormalizeTo0_360(uint16_t par_deg)
{
    uint16_t loc_angle;
    if (par_deg <= THREE_HUNDRED_SIXTY) {
    loc_angle = par_deg;
    } else {
    loc_angle = par_deg % THREE_HUNDRED_SIXTY;
    loc_angle = ((par_deg != 0)?par_deg:THREE_HUNDRED_SIXTY);
    }
    return loc_angle;
}
 ...
 * sweep in degree
 */
void ssd1306_DrawArc(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, SSD1306_COLOR color)
{
    float approx_degree;
    ...
    loc_sweep = ssd1306_NormalizeTo0_360();

    count = (ssd1306_NormalizeTo0_360(start_angle) * CIRCLE_APPROXIMATION_SEGMENTS) / THREE_HUNDRED_SIXTY;
    approx_segments = (loc_sweep * CIRCLE_APPROXIMATION_SEGMENTS) / THREE_HUNDRED_SIXTY;
...
void ssd1306_DrawRegion(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint32_t stride)
{
    uint8_t wp = w;
    uint8_t hp = h;
    uint32_t stride1 = stride;
    if (x + w > SSD1306_WIDTH || y + h > SSD1306_HEIGHT || w * h == 0) {
        printf("%dx%d @ %d,%d out of range or invalid!\r\n", w, h, x, y);
        return;
    }
    wp = (w <= SSD1306_WIDTH ? w : SSD1306_WIDTH);
    hp = (h <= SSD1306_HEIGHT ? h : SSD1306_HEIGHT);
    stride1 = (stride == 0 ? w : stride);

    uint8_t rows = 8 / stride;
